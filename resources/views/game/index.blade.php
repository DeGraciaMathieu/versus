                ...
                @foreach($games as $game)
                    <tr class="bg-secondary-light border-b border-secondary-dark">
                        ...
                        <td class="py-4 px-4 text-left flex items-center justify-end">
                            @can('delete', $game)
                                <form method="POST" action="{{ route('game.destroy', [$ladder, $game]) }}">
                                    @method('delete')
                                    @csrf

                                    <x-link href="{{ route('game.destroy', [$ladder, $game]) }}" onclick="event.preventDefault(); this.closest('form').submit();" class="p-2">
